...
### Blockchain monitoring architecture (M0 — planned; M1 contract; M2 Laravel DB; M3 config; M4 hashing; M5 record service)

The Blockchain Module dashboard is **not implemented** in the frontend (target milestone **M11**). **M5** added backend-only pending `blockchain_records` creation via `BlockchainRecordService` and existing read APIs. The SPA must **not** read `BLOCKCHAIN_*` variables, use Web3/wallet libraries, call Ethereum RPC, or create blockchain records directly. Future dashboard work must call Laravel APIs only.
