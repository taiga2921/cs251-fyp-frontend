...
export default function CheckpointEdit() {
  const { zoneId, checkpointId } = useParams();
  const repository = new CheckpointRepository(checkpointService);
  const controller = useCheckpointFormController(repository, zoneId, checkpointId);

  return (
    <>
      <DetailCard title="Edit Checkpoint Details" avatar={<CheckpointsIcon size={24} />} onBack={controller.handleCancel}>
        <Box sx={{ mx: 'auto' }} component="form" onSubmit={controller.handleSubmit}>
          <Grid container spacing={3}>
            ...
            <Grid size={{ xs: 12, md: 12 }}>
              <SubmitButton text1="Update Checkpoint" text2="Updating..." controller={controller}></SubmitButton>
            </Grid>
          </Grid>
        </Box>
      </DetailCard>
      ...
      {/* Success Dialog */}
      <SuccessDialog controller={controller} msg="Checkpoint updated successfully! Redirecting to checkpoint view page..."></SuccessDialog>
    </>
  );
}
