...
export const usePatrolController = (repository) => {
  ...
  const loadPatrols = async () => {
    try {
      setLoading(true);
      const data = await repository.getAllPatrols();
      setPatrols(data);
    } catch (error) {
  ...
  const handleViewPatrol = (patrolHistoryId) => {
    navigate(`/operator/patrol/history/view/${patrolHistoryId}`);
  };
  ...
  const filteredPatrols = repository.filterPatrols(patrols, filterText);
  const paginatedPatrols = repository.paginatePatrols(filteredPatrols, page, rowsPerPage);
